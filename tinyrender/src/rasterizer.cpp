...
*/
// 替换原有的 rst::rasterizer::rasterizer 函数体
rst::rasterizer::rasterizer(int w, int h, const std::string &format)
    : width(w), height(h), pixel_Mutex(w * h)
{
    image = std::make_unique<Image>(w, h, format);
    depth_buf.resize(w * h, -std::numeric_limits<float>::infinity());
    back_buf.resize(w * h, 0.f);
    
    // --- 关键：在这里分配阴影缓冲区的空间 ---
    shadow_depth_buf.resize(w * h, -std::numeric_limits<float>::infinity()); 
...
*/
// 找到原本的 void rst::rasterizer::draw()，删除它，粘贴以下完整代码：
void rst::rasterizer::draw()
{
    if (scene == nullptr) { LOGE("No scene loaded!"); return; }
    const auto &camera = scene->get_camera();
    if (!camera) { LOGE("No camera in the scene!"); return; }

    // ============================================================
    // Pass 1：Shadow Map 生成（仅在启用阴影时执行）
    // ============================================================
    if (shadow_enabled && !scene->get_lights().empty())
    {
        auto &light = scene->get_lights()[0]; // 目前只支持第一盏灯

        // 计算光源视角的矩阵
        Vec3f light_up = {0.f, 1.f, 0.f};
        Vec3f light_dir = (Vec3f{0.f,0.f,0.f} - light->position).normalize();
        if (std::abs(light_dir * light_up) > 0.99f)
            light_up = {0.f, 0.f, 1.f};

        set_view(light->position, {0.f, 0.f, 0.f}, light_up);
        set_projection(90.f, 1.f, -0.1f, -100.f);

        // 核心：保存光源空间的 MVP
        light_space_mvp = vertex_payload.projection * vertex_payload.view * vertex_payload.model;
        vertex_payload.mvp = light_space_mvp;
        vertex_payload.inv_trans = (vertex_payload.view * vertex_payload.model).inverse().transpose();

        // 准备 Shadow Pass 环境
        std::fill(shadow_depth_buf.begin(), shadow_depth_buf.end(), -std::numeric_limits<float>::infinity());
        auto saved_shader = fragment_shader;
        set_fragment_shader([](const rst::pixel_shader_payload&, rst::rasterizer& ras) -> Vec3f {
            return {0.f, 0.f, 0.f}; // Shadow Pass 不需要计算颜色
        });

        // 临时交换深度图：将渲染结果定向到 shadow_depth_buf
        auto saved_depth = depth_buf;
        depth_buf = shadow_depth_buf;
        ...
        // 还原现场
        shadow_depth_buf = depth_buf;
        depth_buf = saved_depth;
        set_fragment_shader(saved_shader);
    }
    ...
    // ============================================================
    set_view(camera->eye_pos, camera->target_pos, camera->up_dir);
    set_projection(scene->get_filedofView(), scene->get_aspect_ratio(), -1.f, -50.f);

    vertex_payload.mvp = vertex_payload.projection * vertex_payload.view * vertex_payload.model;
    vertex_payload.inv_trans = (vertex_payload.view * vertex_payload.model).inverse().transpose();

    clearBuff(rst::Buffers::Color | rst::Buffers::Depth);
