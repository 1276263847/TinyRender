...
namespace rst
{
    ...
    public:
        ...
        // shadows
        const std::vector<float>& get_shadow_depth_buf() const { return shadow_depth_buf; }
        const Matrix4f& get_light_space_mvp() const { return light_space_mvp; }
        bool is_shadow_enabled() const { return shadow_enabled; }
        void enable_shadow(bool v) { shadow_enabled = v; }
        // 


        // 增加shadows
        void draw_shadow_pass();   // 第一趟：生成深度图 
        void draw_color_pass();    // 第二趟：正常着色
    ...
    private:
        ...
        std::vector<float> shadow_depth_buf;   // 光源视角的深度缓冲
        Matrix4f light_space_mvp;              // 光源空间的MVP矩阵
